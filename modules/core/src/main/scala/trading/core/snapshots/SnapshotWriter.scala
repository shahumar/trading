...
object SnapshotWriter:
  ...
  def from[F[_]: Async: Log](redis: RedisCommands[F, String, String], exp: KeyExpiration): SnapshotWriter[F] = new:
    def saveStatus(st: TradingStatus): F[Unit] =
  ...
  def fromClient[F[_]: Async: Log](client: RedisClient, exp: KeyExpiration): Resource[F, SnapshotWriter[F]] =
