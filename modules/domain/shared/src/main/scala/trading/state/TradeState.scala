...
import trading.domain.TradeAction.Bid
import trading.domain.TradingStatus.On
...
final case class TradeState(
  ...
  def remove(symbol: Symbol)(action: TradeAction, price: Price): TradeState =
    action match
      case TradeAction.Ask =>
        TradeState.__AskPrices(symbol)
          .modify(_.removed(price))(this)
      case Bid => 
        TradeState.__BidPrices(symbol)
          .modify(_.removed(price))(this)

object TradeState:
  def empty: TradeState = TradeState(On, Map.empty)
