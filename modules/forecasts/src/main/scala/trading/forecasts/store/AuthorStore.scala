...
object AuthorStore:
  ...
  def from[F[_]: DoobieTx: MonadCancelThrow](xa: Transactor[F]): AuthorStore[F] = new:
    def fetch(id: AuthorId): F[Option[Author]] =
  ...
  private def transactional[F[_]: MonadCancelThrow](fk: ConnectionIO ~> F): TxAuthorStore[F] = new:
    ...
    def save(author: Author): F[Unit] =
      ...
      val saveForecasts =
        SQL.insertAuthorForecasts(author)
